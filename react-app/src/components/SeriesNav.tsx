...
export default function SeriesNav({ post, locale }: SeriesNavProps) {
  ...
  return (
    <div className="mb-8 rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm dark:bg-muted/20">
      ...
      {total > 1 && (
        <div className="flex flex-wrap items-center gap-1.5">
          {Array.from({ length: total }, (_, i) => {
            ...
            const partSlug = `${seriesSlug}-part-${part}`
            return (
              <Link
                key={part}
      ...
      {total <= 1 && (
        <div className="flex items-center gap-3 mt-1">
          {seriesPart > 1 && (
            <Link
              to={localizePath(`/posts/${seriesSlug}-part-${seriesPart - 1}`, locale)}
