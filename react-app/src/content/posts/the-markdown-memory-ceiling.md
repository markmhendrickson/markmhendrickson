...
The "open the file and edit" workflow returns in a structured form with [Neotoma v0.4.5](https://github.com/markmhendrickson/neotoma/releases/tag/v0.4.5): `neotoma edit <id>` and the [Inspector](https://github.com/markmhendrickson/neotoma-inspector) route typed field edits through `correct()`, so versioning and provenance stay intact without making free-form markdown the source of truth again. For flows that bypass the editor, corrections go through `correct()` directly. Neotoma still costs an install and a running store — friction markdown never had.
...
Structured editing addresses one ergonomic gap. The other is directory-shaped knowledge: `tree mirror/`, `grep -r "Acme Corp" mirror/entities/company/`, and `git diff` against a real tree, not only CLI-shaped approximations. [Neotoma v0.4.5](https://github.com/markmhendrickson/neotoma/releases/tag/v0.4.5) inverts the relationship: markdown stops being canonical and becomes a generated mirror of structured state — `cat`, `grep`, `tree`, `git diff`, and opening a file in an editor as views on an append-only observation log.
