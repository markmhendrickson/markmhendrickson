...
At 100K to 500K tokens (a few days of heavy agent use), retrieval is fine. Boschi says it himself: "at BEAM's 500K tier, the gap between systems shrinks dramatically. Context-stuffing competes with real memory architectures." The BEAM scores confirm it. At 500K, Hindsight scores 71.1% and the RAG baseline scores 33.0%, but that RAG score is almost identical to its 100K performance (32.3%). A 1M context window covers everything. No retrieval architecture needed. But state integrity is already degrading. "Acme Corp" and "ACME CORP" and "Acme Corporation" accumulate as separate entities. Corrections get silently overwritten. The BEAM paper itself found that contradiction resolution scores near zero at every tier, including 100K. All models, including ones with structured memory, struggle to maintain globally consistent state. This is not a scale problem. It is a structural one that starts with the first conflicting write.

At 500K to 2M tokens (a few weeks of multi-tool agent use), the state integrity wall hits. A single Claude Code session consumes 50,000 to 200,000 tokens. A daily power user processes 300K to 2M tokens per day. You reach this tier in days, not months. Entity resolution variants produce genuine confusion across hundreds of references. Cross-session state drift means corrections from last week may or may not persist. Cross-tool fragmentation means the same entity has different representations in Claude, Cursor, and ChatGPT. Platform memory has silently reverted or overwritten state multiple times with no audit trail. This is where users start building workarounds. The OpenClaw ecosystem documents the pattern. SOUL.md holds identity. MEMORY.md holds curated state. HEARTBEAT.md schedules consolidation tasks. Daily log files get distilled by periodic heartbeat jobs. People run markdown CRMs for business development pipelines. They maintain JSON heartbeat files so agents know what changed between sessions. These workarounds exist because the infrastructure does not preserve history. The cost of compensating exceeds tolerance.

At 2M to 10M tokens (months of agent use), both problems compound. The BEAM scores show the retrieval gap widening: at 1M tokens, RAG scores 30.7% and Honcho scores 63.1%. By 10M, RAG drops to 24.9% and Honcho drops to 40.6%. Standard RAG returns chunks about three different "Alices." Even when retrieval finds the right entity, you cannot confirm it is the latest version. Was the correction from March 15 applied? What is the provenance chain? BEAM addresses this tier for retrieval. Nobody benchmarks it for state integrity.

At 10M tokens and beyond, context-stuffing is impossible. Only real memory architecture survives. Hindsight scores 64.1% at this tier. RAG scores 24.9%. That 2.5x gap is structural, not incremental. But without structured state, retrieval has 10M tokens of noise to search through, including every overwritten value and every entity variant that was never resolved.
